...
class Ball {
    // These are the member variables (fields) they all have the m prefix they are all private
    ...
    // This is the constructor method. It is called when by the code: mBall = new Ball(mScreenX);
    // In the PongGame class.
    Ball(int screenX) {
        // Make the ball square and 1% of screen width of the screen width
        mBallWidth = (float)(screenX / 100);
        mBallHeight = (float)(screenX / 100);

        // Initialize the RectF with 0, 0, 0, 0 we do it here because we only want to do it once.
    ...
    void update(long fps) {
        ...
        // vertical(mYVelocity) speed and the current frame rate(mFPS)
    ...
    void reset(float x, float y) {
        ...
        mRect.left = x / 2.0f;
        mRect.top = 0.0f;
        mRect.right = x / 2.0f + mBallWidth;
        mRect.bottom = mBallHeight;

        // How fast will the ball travel you could vary this to suit.
